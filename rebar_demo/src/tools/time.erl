...
get_diff_month(UnixTime1, UnixTime2) ->
    if
        UnixTime1 >= UnixTime2 -> get_diff_month_(UnixTime2, UnixTime1);
        true -> get_diff_month_(UnixTime1, UnixTime2)
    end.

get_diff_month_(UnixTime1, UnixTime2) ->
    {{Year1, Month1, _}, _} = unixtime_to_localtime(UnixTime1),
    {{Year2, Month2, _}, _} = unixtime_to_localtime(UnixTime2),
    if
        Year1 =:= Year2 -> Month2 - Month1;
        true -> (12 - Month1) + 12 * (Year2 - Year1 - 1) + Month2
    end.
...
% 格式化时间戳
% @doc HH:MM
unixtime_to_minute_string(UnixTime) ->
    {{_Year, _Month, _Day}, {Hour, Minute, _Second}} = unixtime_to_localtime(UnixTime),
    lists:flatten(
        io_lib:format(
            "~2..0w:~2..0w",
            [Hour, Minute]
