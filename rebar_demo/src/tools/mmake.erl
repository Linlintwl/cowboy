...
-define(Behaviors,["effect", "bt_node"]).
...
process([{L, Opts} | Rest], Worker, NoExec, Load) ->
    Parfun = fun(Name) ->
        ...
        lists:member(BaseName, ?Behaviors)
    end,
    {Behaviors, Normal} = lists:partition(Parfun, L),

    CompiledResB = if
        Behaviors =/= [] -> catch do_worker(Behaviors, Opts, NoExec, Load, 1);
        true -> ok
    end,
    CompiledResN = if
        CompiledResB =:= ok ->
            Len = length(Normal),
            SpawnWorkers = erlang:min(Len, Worker),
            catch do_worker(Normal, Opts, NoExec, Load, SpawnWorkers);
        true ->
            CompiledResB
    end,
    if
        CompiledResN =:= ok -> process(Rest, Worker, NoExec, Load);
        true -> CompiledResN
    end;
